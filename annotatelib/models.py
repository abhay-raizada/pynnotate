...
def get_model_path_info_map(model_path, config_path):
    ...
    for model_path in models(model_path):
        ...
        columns_description = _get_column_description_from_object(schema_manager, table_name)
        if not columns_description:
            continue
        indices_description = _get_indices_description_from_oject(schema_manager, table_name)
